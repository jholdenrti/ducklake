...
SinkFinalizeType DuckLakeMergeInsert::Finalize(Pipeline &pipeline, Event &event, ClientContext &context,
	...
	// FinalizeCopyToInsert now drives copy.Finalize internally (and pumps the event it may create on newer
	// DuckDB), so we no longer call copy.Finalize ourselves here.
...
SinkFinalizeType DuckLakeMergeUpdate::Finalize(Pipeline &pipeline, Event &event, ClientContext &context,
	...
	// FinalizeCopyToInsert now drives copy_op.Finalize internally (and pumps the event it may create on newer
	// DuckDB), so we no longer call copy_op.Finalize ourselves here.
